...
Hour nodes come from the scheduled tick, not from session writes. No segments in the window means no Hour node — the hierarchy stays sparse. The tick skips only an Hour that still contains an **open segment**. Scheduler: `tick-hour --period $(date -u +%Y-%m-%dT%H)` once an hour; that invocation also revisits any earlier un-finalized Hour in the resume worklist. Segment artifacts live in the Hour they belong to.
